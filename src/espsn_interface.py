...
from espsn_experiment_data import ESPSNExperimentData
import espsn_util
...
def train_weight_and_reg_coef_search(experimant_data, reg_coefs=np.arange(0.1, 2.0, 0.1)):
    ...
    for reg_coef in reg_coefs:
        ...
        espsn_util.print_status("reg_coef: %f  / MSE: %f" % (reg_coef, mse), header="")
...
if __name__ == '__main__':
    ...
    espsn_util.print_status("reading settings and data...")
    data = ESPSNExperimentData(sys.argv[1], sys.argv[2])

    espsn_util.print_status("training wegihts...")
    ...
    espsn_util.print_status("saving result...")
